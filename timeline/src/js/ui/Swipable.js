...
export default class Swipable {
	...
	constructor(drag_elem, move_elem, options) {
		...
		if (this.usePointerEvents) {
			this.dragevent = this.pointerdrag;
		} else if (BROWSER_TOUCH) {
	...
	_onDragStart(e) {
		...
		if (this.usePointerEvents) {
			this.activePointerId = typeof e.pointerId === "number" ? e.pointerId : null;
			this.data.pagex.start = e.clientX;
			this.data.pagey.start = e.clientY;
			if (this.activePointerId !== null && typeof this._el.drag.setPointerCapture === "function") {
				this._el.drag.setPointerCapture(this.activePointerId);
			}
		} else if (BROWSER_TOUCH) {
			if (e.originalEvent) {
		...
		this.data.touch = this.usePointerEvents ? e.pointerType === "touch" : BROWSER_TOUCH;
	...
	_onDragEnd(e) {
		if (this.usePointerEvents) {
			if (this.activePointerId !== null && typeof e.pointerId === "number" && e.pointerId !== this.activePointerId) {
				return;
			}
		}
		// If no move happened, use starting coords to avoid huge momentum jump
		if (this.data.pagex.end === undefined) {
			this.data.pagex.end = this.data.pagex.start;
		}
		if (this.data.pagey.end === undefined) {
			this.data.pagey.end = this.data.pagey.start;
		}
		...
		if (this.usePointerEvents && this.activePointerId !== null && typeof this._el.drag.releasePointerCapture === "function") {
			this._el.drag.releasePointerCapture(this.activePointerId);
		}
		this.activePointerId = null;
	...
	_onDragMove(e) {
		...
		if (this.usePointerEvents) {
			...
			this.data.pagex.end = e.clientX;
			this.data.pagey.end = e.clientY;
		} else if (BROWSER_TOUCH) {
			if (e.originalEvent) {
